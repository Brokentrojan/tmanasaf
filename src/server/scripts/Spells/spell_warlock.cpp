	...
	class spell_warl_everlasting_affliction_SpellScript : public SpellScript
	{
		...
		void HandleScriptEffect(SpellEffIndex /*effIndex*/)
		{
			...
			if (Unit* target = GetHitUnit())
				...
				{
					aur->SetDamage(caster->SpellDamageBonusDone(target, aur->GetSpellInfo(), 0, DOT));
...
class spell_warlock_consume_shadows : public SpellScriptLoader
{
public:
	spell_warlock_consume_shadows() : SpellScriptLoader("spell_warlock_consume_shadows") { }

	class spell_warlock_consume_shadows_SpellScript : public SpellScript
	{
		PrepareSpellScript(spell_warlock_consume_shadows_SpellScript);
		...
		void Register()
		{
			BeforeCast += SpellCastFn(spell_warlock_consume_shadows_SpellScript::HandleBeforeCast);
		}
	};

	class spell_warlock_consume_shadows_AuraScript : public AuraScript
	{
		PrepareAuraScript(spell_warlock_consume_shadows_AuraScript);
		...
		void Register()
		{
			OnEffectRemove += AuraEffectRemoveFn(spell_warlock_consume_shadows_AuraScript::HandleOnEffectRemove, EFFECT_1, SPELL_AURA_PROC_TRIGGER_SPELL, AURA_EFFECT_HANDLE_REAL);
		}
	};

	SpellScript* GetSpellScript() const
	{
		return new spell_warlock_consume_shadows_SpellScript();
	}

	AuraScript* GetAuraScript() const
	{
		return new spell_warlock_consume_shadows_AuraScript();
	}
};
...
void AddSC_warlock_spell_scripts()
{
	...
	new spell_warlock_consume_shadows();
}
